    ...
    class BedCalibrationSuitePlugin(
        ...
        def __init__(self):
            ...
            self._bedlevelvisualizer_logger = logging.getLogger(
                "octoprint.plugins.bedcalibrationsuite.debug"
            )
            ...
            # Mesh collection state (ported from octoprint_bedlevelvisualizer)
            self.mesh = []
            self.box = []
            self.bed = {}
            self.bed_type = None
            ...
            # Regex patterns (ported verbatim)
            ...
            self.regex_bed_temp = re.compile(r"B:\s*([-\d.]+)\s*/\s*[-\d.]+")
            ...
            self.regex_unknown_command = re.compile(
                r"echo:Unknown command: \"@BEDCALIBRATIONSUITE\""
            )
        ...
        # --- Mesh collection (ported from octoprint_bedlevelvisualizer) ---
        ...
        def flag_mesh_collection(self, comm_instance, phase, command, parameters, tags=None, *args, **kwargs):
            if command == "BEDCALIBRATIONSUITE" or command == "BEDLEVELVISUALIZER":
                if parameters:
        ...
        def process_gcode(self, comm, line, *args, **kwargs):
            if self.printing and line.strip() in ("echo:BEDCALIBRATIONSUITE", "echo:BEDLEVELVISUALIZER"):
                thread = threading.Thread(target=self.enable_mesh_collection)
                thread.daemon = True
                thread.start()
                return line
            ...
            if ("ok" in line or (self.repetier_firmware and "T:" in line)) and len(
                ...
                # Update bed_health with mesh stats (bedcalibrationsuite-specific)
                if self._health and self._mesh_processor:
                    try:
                        stats = self._mesh_processor.compute_stats(self.mesh)
                        self._health.add_mesh(
                            stats["variance_mm"],
                            stats["max_deviation_mm"],
                            self.mesh,
                        )
                        trend = self._health.get_mesh_trend()
                        mesh_score = self._health.compute_mesh_score(
                            stats["variance_mm"], trend
                        )
                        self._health.state["score_breakdown"]["mesh"] = round(mesh_score)
                        self._health.save()
                    except Exception as e:
                        self._logger.warning(
                            "Failed to update bed_health after mesh collection: {}".format(e)
                        )
        ...
        def create_circular_mask(self, y, x):
            ...
            mask = [[False for j in range(x)] for i in range(y)]
            for i in range(y):
                for j in range(x):
        ...
        def send_mesh_data_collected_event(self, mesh_data, bed_data):
            event = Events.PLUGIN_BEDCALIBRATIONSUITE_MESH_DATA_COLLECTED
        ...
        def custom_action_handler(self, comm, line, action, *args, **kwargs):
            if not action == "BEDCALIBRATIONSUITE_LEVELBED":
                return
            self._bedlevelvisualizer_logger.debug("Received BEDCALIBRATIONSUITE_LEVELBED command.")
